...
static int _dns_client_tls_matchName(char *pattern, char *host)
{
	// Credit to curl
	...
	if (pattern_wildcard == NULL)
		return strcasecmp(pattern, host);
	...
	if (pattern_label_end == NULL || strchr(pattern_label_end + 1, '.') == NULL ||
		pattern_wildcard > pattern_label_end || !strncasecmp(pattern, "xn--", 4)) {
		wildcard_enabled = 0;
	}
	if (!wildcard_enabled)
		return strcasecmp(pattern, host);

	hostname_label_end = strchr(host, '.');
	...
	/* The wildcard must match at least one character, so the left-most
	   ...
	   of the pattern. */
	if (hostname_label_end - host < pattern_label_end - pattern)
		return -1;

	prefixlen = pattern_wildcard - pattern;
	suffixlen = pattern_label_end - (pattern_wildcard + 1);
	return !strncasecmp(pattern, host, prefixlen) &&
				   !strncasecmp(pattern_wildcard + 1, hostname_label_end - suffixlen, suffixlen)
			   ? 0
			   : -1;
}
...
static int _dns_client_tls_verify(struct dns_server_info *server_info)
{
	...
	if (tls_host_verify) {
		if (_dns_client_tls_matchName(peer_CN, tls_host_verify) != 0) {
